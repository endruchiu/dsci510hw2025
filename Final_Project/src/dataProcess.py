...
def getUSSoftwareDeveloperSalariesData(out_file=DATA_US_SOFTWARE_DEVELOPER_SALARIES, force_download=True):
    ...
    try:
        ...
        path = kagglehub.dataset_download(URL_KAGGLEAPI_US_SOFTWARE_DEVELOPER_SALARIES, force_download=force_download)
...
def getAnnualWageStatisticsData(out_file=DATA_ANNUAL_WAGE_STATISTICS):
    ...
    url = URL_ANNUAL_WAGE_STATISTICS # alternative data source
...
def getSoftwareEngineerSalaryData(out_file=DATA_SOFTWARE_ENGINEER_SALARY, remove_cache=True):
    ...
    if not os.path.exists('tmp.html'):
        ...
        url_menu = URL_SOFTWARE_ENGINEER_SALARY_MENU
        try:
    ...
    for city in slugs:
        url = f'{URL_SOFTWARE_ENGINEER_SALARY_SEARCH_BASE}/{city}'
